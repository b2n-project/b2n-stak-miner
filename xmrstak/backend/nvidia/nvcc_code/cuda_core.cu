...
void cryptonight_core_cpu_hash(nvid_ctx* ctx, xmrstak_algo miner_algo, uint32_t startNonce)
{
	...
	else if(miner_algo == cryptonight_alloy)
	{
		...
		else if(miner_algo == cryptonight_b2n)
	{
		cryptonight_core_gpu_hash<CRYPTONIGHT_B2N_ITER, CRYPTONIGHT_B2N_MASK, CRYPTONIGHT_B2N_MEMORY/4, cryptonight_b2n>(ctx, startNonce);
	}
}
